                    ...
                    <table className="w-full text-left">
                        ...
                        <tbody>{filteredTransactions.length > 0 ? (
                            ...
                            <tr key={t.id} className="border-b opacity-80">
                                ...
                                <td className={`py-4 text-right font-bold ${t.amount > 0 ? 'text-green-500' : 'text-red-500'}`}>{t.amount > 0 ? '+' : ''}₹{Math.abs(t.amount).toLocaleString()}</td>
                            </tr>
                        ))) : (
                            <tr><td colSpan={4} className="py-10 text-center opacity-50"><Inbox className="mx-auto mb-2" /> No {activePage.toLowerCase()} found</td></tr>
                        )}</tbody>
                    </table>
                </div>
            )}
